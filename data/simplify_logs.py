...
def parse_log(logname, mcu, recent=None):
    if mcu is None:
        mcu = "mcu"
    mcu_prefix = mcu + ":"
    ...
    for line in f:
        ...
        # If there's a big jump in sampletime, reset the data
        if last_sampletime == 0.0:
            last_sampletime = keyparts['#sampletime']
        if recent and keyparts['#sampletime'] - last_sampletime > recent:
            print("Resetting data due to large sampletime jump", keyparts['#sampletime'])
            out = []
        last_sampletime = keyparts['#sampletime']
        out.append(keyparts)
    f.close()
...
def plot_mcu(data, maxbw):
    ...
    for d in data:
        ...
        if st - basetime < 15.:
            load = 0.
        retransmit = float(d['bytes_retransmit'])
    ...
    ax1.plot_date(times, bwdeltas, 'g', label=f'Bandwidth max: {max(bwdeltas)}', alpha=0.8)
    ax1.plot_date(times, loads, 'r', label='MCU load', alpha=0.8)
    ax1.plot_date(times, retransmits, 'm', label=f'Retransmit max: {max(retransmits)}', alpha=0.8)
    ax1.plot_date(times, hostbuffers, 'c', label=f'Host buffermax: {max(hostbuffers)}', alpha=0.8)
